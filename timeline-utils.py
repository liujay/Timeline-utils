...
def parse_timeline_json(infile):
    ...
    for segidx, seg in enumerate(segments):
        if 'timelinePath' in seg.keys():
            ...
            try:
                raw_coords = seg["visit"]["topCandidate"]["placeLocation"]["latLng"].replace("°", "").strip()
                coords = raw_coords.split(", ")
                lat, lon = coords[0], coords[1]
            ...
            try:
                startRaw_coords = seg["activity"]["start"]["latLng"].replace("°", "").strip()
                startCoords = startRaw_coords.split(", ")
                endRaw_coords = seg["activity"]["end"]["latLng"].replace("°", "").strip()
